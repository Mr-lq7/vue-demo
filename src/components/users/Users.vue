 ...
 //     this.userlist = data.data.users;
    ...
    editUser() {
      this.$refs.editUserFormRef.validate(async valid => {
        ...
        // });
        const {data} = await this.$http.put('users',{email:this.editUserForm.email,mobile:this.editUserForm.mobile,id:this.queryInfo.id})
        console.log(data)
        if (data.data !== '200') {
    ...
    //       // const { data } = await this.$http.delete(`users`+id);
    //       console.log(id)
    //       const { data } = await this.$http.delete('users',{id:id});
    //       console.log(data);
    //       if (data.data !== 200) {
    //         this.$message({
    //           type:"warning",
